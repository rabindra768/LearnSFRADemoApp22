...
function selectShippingMethod(shipment, shippingMethodID, shippingMethods, address) {
    ...
    var testing =addCustomShipRestriction(shipment);
    ...
    if (!isShipmentSet) {
        if (collections.find(applicableShippingMethods, function (sMethod) {
            ...
            var firstMethod = getFirstApplicableShippingMethod(applicableShippingMethods[0], true);
...
/**
 * Plain JS object that represents a DW Script API object
 * @param {dw.order.Basket} currentBasket - the target Basket object
 */
 function getProductCustTypeInBasket(currentBasket) {
    var customProductType = 'normal';
    for (var i = 0; i < currentBasket.allProductLineItems.length; i++) {
        var pli = currentBasket.allProductLineItems[i];
        if (pli.product.custom && pli.product.custom.is_product_hazmat) {
            customProductType = 'is_product_hazmat';
            break;  
        } else if (pli.product.custom && (pli.product.custom.is_product_monogrammable)) {
            customProductType = 'is_monogram_or_engrave';
            break; 
        } else if (pli.product.custom && pli.product.custom.is_product_exotic) {
            customProductType = 'is_product_exotic';
            break;
        } else {
            customProductType = 'normal';
        }
    }
 ...
 * @param {Object} [address] - optional address object
 */
function addCustomShipRestriction(address) {
    var customProdShipRestrict = Site.getCurrent().getCustomPreferenceValue('customProductAttributeShipRestriction');
    customProdShipRestrict = JSON.parse(customProdShipRestrict);
    var currentBasket = BasketMgr.getCurrentBasket();
    var productType = getProductCustTypeInBasket(currentBasket);
    var shipMethods = [];
    for (var j = 0; j < customProdShipRestrict.length; j++) {
        var stateCodeVal = '';
        var status = false;
        if (customProdShipRestrict[j].attribute_id == productType && customProdShipRestrict[j].value == 'true') {
            productType = customProdShipRestrict[j].attribute_id;
            var custShippingMethod = customProdShipRestrict[j].exclusions[j].shipping_method_ids;
            var restrictStateCode = customProdShipRestrict[j].exclusions[j].stateCode;
            restrictStateCode = restrictStateCode.split(',');
            var countryCode = customProdShipRestrict[j].exclusions[j].countryCode;
            var st = restrictStateCode.includes(address.stateCode);
            if (!!restrictStateCode && restrictStateCode.includes(address.stateCode)) {
                stateCodeVal = address.stateCode;
                status = true;
            }
            shipMethods.push({ status: status, prod: productType, stateCode: stateCodeVal, countryCode: countryCode });
            break;
        } 
    }
    return shipMethods;
...
module.exports = {
    ...
    addCustomShipRestriction:addCustomShipRestriction
};
